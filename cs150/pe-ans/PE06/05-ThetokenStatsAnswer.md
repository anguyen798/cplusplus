...
void tokenStats(istream & in, ostream & out) {
   string line, longestLine;
   int lineCount {0}, longestLineLength {0};
   ...
   while (getline(in, line))
   {
      ...
      string token;
      int tokenCount {0}, longestTokenLength {0};
      
      while (iss >> token)
      {
         tokenCount++;
         if (token.size() > longestTokenLength) { longestTokenLength = token.size(); }
      }
      ...
      out << "Line " << lineCount << " has " << tokenCount << " tokens (longest = " << longestTokenLength << ")\n";
   }
   
   out << "Longest line: " << longestLine << "\n";
}
