...
void triathalon(istream & in, ostream & out)
{
   string name;
   int swim_time, bike_time, run_time, winner_time {0};

   while (in >> name >> swim_time >> bike_time >> run_time)
   {
      int total_time = swim_time + bike_time + run_time;
      if (winner_time == 0) // For the first entry, winner_time will be zero
      {
         winner_time = total_time;
         out << name << ": " << total_time << " min\n";
      }
      else {
         out << name << ": " << total_time << " min (+" <<
            (total_time - winner_time) << " min)\n";
      }
   }
}
